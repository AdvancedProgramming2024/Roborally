...
public class PushPanel extends FieldAction {
    ...
    public boolean doAction(@NotNull GameController gameController, @NotNull Space space) {
        ...
        if (pushTime == PushTime.EVEN && space.board.getStep() % 2 + 1 == 0) {  // +1 because step is 0-indexed
            gameController.moveInDirection(player, heading);
        } else if (pushTime == PushTime.ODD && space.board.getStep() % 2 + 1 == 1) {  // +1 because step is 0-indexed
            gameController.moveInDirection(player, heading);
        }
        return true;
    }
